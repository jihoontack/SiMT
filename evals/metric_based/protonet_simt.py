...
def test_classifier(P, model, loader, criterion, steps, logger=None):
    ...
    for n, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            test_embeddings_ema = P.moving_average(test_inputs)

        prototypes = get_prototypes(train_embeddings, train_targets, P.num_ways)
        squared_distances = - torch.sum((prototypes.unsqueeze(2)
                                         - test_embeddings.unsqueeze(1)) ** 2, dim=-1)

        prototypes_ema = get_prototypes(train_embeddings_ema, train_targets, P.num_ways)
        ...
        loss_ema = criterion(-squared_distances_ema, test_targets)
        acc_ema = get_accuracy(prototypes_ema, test_embeddings_ema, test_targets).item()
    ...
    log_(' * [AccEMA@1 %.3f] [LossOutEMA %.3f]' %
